#!/usr/bin/env python
"""Command-line entry point to synchronise QBench samples."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Synchronise QBench samples")
    parser.add_argument(
        "--full",
        action="store_true",
...
def main() -> None:
    ...
    logging.info("Starting sample sync (full_refresh=%s)", args.full)
    ...
    logging.info("Sample sync completed: %s", pformat(summary.__dict__))
    if summary.skipped_entities:
        logging.warning("Samples skipped (%d):", len(summary.skipped_entities))
        for line in summarize_skipped_entities(summary.skipped_entities):
            logging.warning("  %s", line)
    else:
        logging.info("No samples were skipped during the sync.")
