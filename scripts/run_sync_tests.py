#!/usr/bin/env python
"""Command-line entry point to synchronise QBench tests."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Synchronise QBench tests")
    parser.add_argument(
        "--full",
        action="store_true",
...
def main() -> None:
    ...
    logging.info("Starting test sync (full_refresh=%s)", args.full)
    ...
    logging.info("Test sync completed: %s", pformat(summary.__dict__))
    if summary.skipped_entities:
        logging.warning("Tests skipped (%d):", len(summary.skipped_entities))
        for line in summarize_skipped_entities(summary.skipped_entities):
            logging.warning("  %s", line)
    else:
        logging.info("No tests were skipped during the sync.")
