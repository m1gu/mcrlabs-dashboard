#!/usr/bin/env python
"""Command-line entry point to synchronise QBench batches."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Synchronise QBench batches")
    parser.add_argument(
        "--full",
        action="store_true",
    ...
    parser.add_argument(
        "--include-worksheet",
        action="store_true",
        help="Include raw worksheet data for each batch",
    )
...
def main() -> None:
    ...
    logging.info("Starting batch sync (full_refresh=%s)", args.full)
    ...
    with tqdm(
        ...
        summary = sync_batches(
            ...
            include_raw_worksheet_data=args.include_worksheet,
            progress_callback=progress_callback,
        )
    if disable_progress:
        logging.info("Processed %s pages", summary.pages_seen)
    logging.info("Batch sync completed: %s", pformat(summary.__dict__))
    if summary.skipped_entities:
        logging.warning("Batches skipped (%d):", len(summary.skipped_entities))
        for line in summarize_skipped_entities(summary.skipped_entities):
            logging.warning("  %s", line)
    else:
        logging.info("No batches were skipped during the sync.")
