...
import type { OverviewFilters, TimeframeOption } from './types'
...
export function OverviewTab() {
  ...
  const { data, loading, error, refresh } = useOverviewData(filters)
  ...
  return (
    <div className="overview">
      ...
      <section className="overview__kpis">
        ...
        <KpiCard
          label="Reports"
          value={formatNumber(data?.summary.reports ?? null)}
          subtitle={
            data
              ? `Within SLA: ${formatNumber(data.reports.within)} | Beyond SLA: ${formatNumber(data.reports.beyond)}`
              : undefined
          }
        />
      ...
      <section className="overview__grid">
        ...
        <div className="overview__card">
          <CardHeader title="Types of tests most requested" subtitle="Distribution by test label" />
          <div className="overview__chart overview__chart--compact">
            {data?.testsByLabel?.length ? (
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={data.testsByLabel} layout="vertical" margin={{ left: 0 }}>
                  ...
                  <Tooltip
                    ...
                    labelStyle={{ color: '#f4f7ff' }}
                  />
                  <Bar dataKey="count" name="Tests" fill="#7EE787" radius={[0, 8, 8, 0]} barSize={18} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <EmptyState loading={loading} />
        ...
        <div className="overview__card overview__card--full">
          <CardHeader title="Daily TAT trend" subtitle="Average hours and SLA compliance per day" />
          <div className="overview__chart">
              ...
              <ResponsiveContainer width="100%" height={340}>
                <AreaChart data={data.tatDaily}>
                  ...
                  <Line
                    ...
                    name="Moving avg (h)"
                  />
                  <ReferenceLine yAxisId="hours" y={48} stroke="#FFD166" strokeDasharray="5 5" label="SLA 48h" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <EmptyState loading={loading} />
